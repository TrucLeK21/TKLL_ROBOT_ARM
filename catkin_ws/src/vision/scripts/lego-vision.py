...
legoClasses = ['X1-Y4-Z2', 'X2-Y2-Z2', 'X2-Y2-Z2-FILLET']
...
def process_item(imgs, item):
    ...
    print("---------------")
    for value in (x1, y1, x2, y2, cn, cl, nm):
    	print(value)
    print("---------------")
    mar = 15
    ...
    if a_show:
        ...
        cv.circle(img_draw, np.int0(top_box[iver]),1, (0,255,0),5,cv.LINE_AA)    
